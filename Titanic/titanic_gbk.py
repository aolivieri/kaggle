...
#le2 = LabelEncoder()
#full["Embarked"] = le2.fit_transform(full["Embarked"])
...
# Predict!
test['Survived'] = gbk.predict(test.drop(['PassengerId'], axis = 1))

# Cast 'Survived' back to integer
test['Survived'] = test['Survived'].astype(np.int8)

# Write test predictions for final submission
test[['PassengerId', 'Survived']].to_csv('kaggle_submission_titanic3.csv', index = False)

my_submission = pd.read_csv('kaggle_submission_titanic3.csv')
